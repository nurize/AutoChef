...
const ContactPage = () => {
  // Memoize contact elements to prevent re-creation on each render
      ...
      <div className="bg-white min-h-screen flex flex-col py-16 px-4 w-9/12 mx-auto">
        ...
        <div className="flex space-x-6 mb-8">
          {socialMediaIcons.map(({ Icon, key, link }) => (
            <a href={link} target='blank' key={key}>
              <Icon className="text-black text-2xl cursor-pointer hover:text-red-600 transition duration-300" />
            </a>
          ))}
        </div>
        <div className="mb-16">
